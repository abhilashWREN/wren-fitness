...
<html class="no-js"> 
	...
	<body>
	<div id="event-dialog-form" style="display:none;">
  <form id="vieweventForm" action="/wren-fitness/trainee/registerEvent" method="post" >
    <fieldset>
    <table>
    <tr> <td><label for="name">Event Title:</label></td>
    	 <td><input type="text" name="title" id="title" class="text ui-widget-content ui-corner-all" readonly/></td>
    </tr>
    <tr>
    	<td><label for="allDay">All Day Event</label></td>
    	<td><input type="checkbox" value="allDayEvent" name="allDay" id="allDay" class="text ui-widget-content ui-corner-all" disabled></td>
    </tr>
    <tr>
    ...
    <tr>
    	...
    	<td> <input type="text" id="address" name="address" class="text ui-widget-content ui-corner-all" readonly/></td>
    </tr>
    <tr>
    	<td> <label for="capacity">Capacity</label></td>
    	<td> <input type="text" id="capacity" name="capacity" class="text ui-widget-content ui-corner-all" readonly/></td>
    </table>
     ...
     <input type = "button" id= "registerbtn" onclick="registerEvent()" value="Register">  
</div>
							...
							<ul class="sf-menu" id="fh5co-primary-menu" >
								...
								<li><a href="#" class="fh5co-sub-ddown">User</a>
									<ul class="fh5co-sub-menu" >
										...
										<li><a href="/wren-fitness/logout" target="_blank">Logout</a></li>
									</ul>
								</li>
	...
	var allEvent = [];
			...
			<c:if test="${fn:length(events) gt 0}">
			{
				...
				allDay : '<c:out value='${event.allDay}'/>',
			 	...
			 	id: '<c:out value='${event.id}'/>',
					...
					eventClick:  function(event, jsEvent, view) {
				        ...
				        openEventDialog(event,allEvent);
				...
				<c:if test="${fn:length(events) gt 0}">
				{
					...
					var id = '${event.id}';
					 ...
					 eventArr.push('${event.maxCapacity}');
					 eventArr.push('${event.currCapacity}');
					 obj[id] = eventArr
					 allEvent.push(obj);
			...
			function checkEventCapacity(event){
				for(var i in allEvent){
			        ...
			        <c:forEach items="${registerEvent}" var="registered" varStatus="status">
			        var registerId = '${registered.id}';
